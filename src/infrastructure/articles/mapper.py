...
def convert_db_to_article_dto(
    ...
    return ArticleDTO(
        ...
        author=UserDTO(
            ...
            nickname=db_article.author.nickname,
            full_name=f"{db_article.author.name} {db_article.author.lastname} {db_article.author.patronymic}",
        ),
        tags=[convert_db_to_tag_dto(db_tag) for db_tag in db_tags],
        is_visible=db_article.is_visable,
