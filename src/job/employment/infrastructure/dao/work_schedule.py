...
class WorkScheduleDAO:
    ...
    async def map_work_schedules_to_cv(
        self, cv_id: UUID, work_schedules_id: list[UUID],
    ) -> None:
    ...
    async def unmap_work_schedules_from_cv(self, cv_id: UUID) -> None:
        await self._base.execute(
            delete(self._rel_work_schedule_cv).where(
                self._rel_work_schedule_cv.cv_id == cv_id,
            ),
        )

    async def update_work_schedules_for_cv(
        self, cv_id: UUID, work_schedules_ids: list[UUID],
    ) -> None:
