...
class WorkFormatDAO:
    ...
    async def map_work_formats_to_cv(
        self, cv_id: UUID, work_formats_id: list[UUID]
    ) -> None:
    ...
    async def unmap_work_formats_from_cv(self, cv_id: UUID) -> None:
        await self._base.execute(
            delete(self._rel_work_format_cv).where(
                self._rel_work_format_cv.cv_id == cv_id
            ),
        )

    async def update_work_formats_for_cv(
        self, cv_id: UUID, work_formats_ids: list[UUID]
    ) -> None:
