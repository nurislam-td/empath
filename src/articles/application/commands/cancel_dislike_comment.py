...
class CancelDislikeComment(Command[None]):
    id: UUID
    user_id: UUID


@dataclass
class CancelDislikeCommentHandler(CommandHandler[CancelDislikeComment, None]):
    ...
    async def __call__(self, command: CancelDislikeComment) -> None:
        comment = await self._comment_reader.get_comment_by_id(
            comment_id=command.id, user_id=command.user_id,
        )
        await self._comment_repo.cancel_dislike_comment(
            comment_id=command.id, user_id=command.user_id,
        )
